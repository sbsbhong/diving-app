...
이 앱은 certified dive computer, 감압 컴퓨터, 의료기기, 생명 유지 장치가 아니다. 현재 화면은 watch에서 기록한 세션, 로컬 확인, 모바일 로그북 미리보기, 계획 알림, 기억/공유 미리보기에 집중한다.

현재 사용하는 workspace는 다음과 같다.

- `apps/mobile`: React Native `0.85.3` 앱. 자체 tab navigation, local in-memory logbook state, watch fixture 가져오기, 계획 알림, 기억 미리보기 화면을 포함한다.
